...
@Component({
  ...
  template: `
    <div [attr.data-testid]="'product-card-' + product().id"
      ...
      <div class="flex items-center gap-1 mb-1">
        ...
        <span class="text-[11px] font-semibold text-text-secondary">{{ product().rating ?? 4.5 }}</span>
      </div>
      ...
      <div class="flex items-center justify-between">
        <span class="text-[16px] font-extrabold text-text-primary">₹{{ priceLabel() }}</span>
        <button [attr.data-testid]="'add-product-' + product().id" type="button"
...
export class ProductCardComponent {
  ...
  toggleFav(): void { this.cart.toggleFavorite(this.product().id); }
}
